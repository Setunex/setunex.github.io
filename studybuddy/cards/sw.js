...
const FILES = ['./quiz.html', './styles.css', './app.js', './generator.js', './manifest.json', OFFLINE_URL, './icon.svg'];
...
self.addEventListener('fetch', event => {
	...
	// Network-first for same-origin API or navigation requests (freshness)
	...
	event.respondWith(
		caches.match(request).then(
			cached =>
				...
				fetch(request)
					.then(resp => {
						// populate runtime cache for non-static responses
					...
					.catch(() => {
						// if a navigation or HTML request fails, show offline fallback
